...
#define SMARC_ID_PAD_CTRL (PAD_CTL_HYS | PAD_CTL_DSE1 | PAD_CTL_PE)
...
int var_detect_board_id(void)
{
	...
	if (board_id == BOARD_ID_UNDEF) {
		...
		board_id = gpio_get_value(SMARC_DETECT_GPIO) ?
			BOARD_ID_SMARC : BOARD_ID_DART;
